...
def timeline_figure(aggregated_data: pd.DataFrame,
    ...
    **kwargs : typing.Any
    ...
    figure.add_scatter(customdata=data.loc[:, ["name", "date"]].values,
                       ...
                       # position the dots by week
                       x=data[kwargs.get("scatter_x")],
...
def days(original: pd.DataFrame,
    ...
    **kwargs : typing.Any
    ...
    plot_title = "Weekdays"
    # prepare data
    data = original.groupby(["app", "weekday"])["id"].count().reset_index()
    data["percentage"] = data["id"] / original.shape[0]
    # create figure
...
def weekdays_figure(aggregated_data: pd.DataFrame,
    ...
    **kwargs : typing.Any
    ...
    figure.update_layout(xaxis={"tickmode": "array",
                                ...
                                "ticktext":
                                    ["M", "T", "W", "T", "F", "S", "S"],
                                # fix that all labels are shown even if the
                                # column is empty
                                "range": [-.5, 6.5]
                                },
...
def sunburst_figure(aggregated_data: pd.DataFrame,
    ...
    **kwargs : typing.Any
    ...
    figure = px.sunburst(data_frame=aggregated_data,
                         ...
                         values="counts",
                         color_discrete_sequence=backend.DISCRETE_COLOR_R,
                         title=title,
                         template=backend.TEMPLATE,
                         height=height,
                         **kwargs)
...
def worldmap_figure(data: pd.DataFrame,
    ...
    **kwargs : typing.Any
    ...
    -------
    figure : go.Figure
        The plotly line mapbox of the locations and routes overlayed with the
        scatter plot.
    ...
    figure = px.choropleth_mapbox(data_frame=countries,
                                  ...
                                  hover_data=["country"],
                                  color_continuous_scale=backend.DISCRETE_COLOR,
